...
export const handleCreate = async (req: AuthenticateRequest, res: Response) => {
    ...
    try {
        ...
        const expense = await ExpenseService.getExpenseById(expenseIdNum);
        if (!expense) {
        ...
        const contributions = (expense as any).Contributions as Array<{ groupMemberId: number; value: number }>;
        const totalFromOthers = contributions
            .filter(c => c.groupMemberId !== groupMemberId)
            .reduce((sum, c) => sum + c.value, 0);
        const maxAllowed = expense.value - totalFromOthers;

        if (valueFloat > maxAllowed) {
            return res.status(400).json({
                message: `Contribution cannot exceed the remaining balance of ${maxAllowed.toFixed(2)} ${expense.currency}.`
            });
        }
...
export const handleUpdate = async (req: AuthenticateRequest, res: Response) => {
    ...
    try {
        ...
        if (contribution.member.userId !== authenticatedUserId) {
            return res.status(403).json({
