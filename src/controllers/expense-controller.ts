...
export const handleCreate = async (req: AuthenticateRequest, res: Response) => {
    ...
    const currentMember = await GroupMemberService.getMemberByUserAndGroup(authenticatedUserId, groupIdNum);
    if (!currentMember) {
...
export const handleGetAll = async (req: AuthenticateRequest, res: Response) => {
    ...
    const currentMember = await GroupMemberService.getMemberByUserAndGroup(authenticatedUserId, groupId);
    if (!currentMember) {
        return res.status(403).json({
            message: "You are not a member of this group."
        });
...
export const handleUpdate = async (req: AuthenticateRequest, res: Response) => {
    ...
    try {
        ...
        const currentMember = await GroupMemberService.getMemberByUserAndGroup(authenticatedUserId, expense.groupId);
        if (!currentMember || !currentMember.isAdmin) {
            return res.status(403).json({
                message: "Only group admins can update expenses."
            });
        }
...
export const handleDelete = async (req: AuthenticateRequest, res: Response) => {
    ...
    try {
        ...
        if (!currentMember || !currentMember.isAdmin) {
            return res.status(403).json({
                message: "Only group admins can delete expenses."
            });
        }
