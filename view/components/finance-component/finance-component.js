document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentMemberId = parseInt(container.dataset.currentMemberId);
  ...
  container.addEventListener("click", async (e) => {
    ...
    if (e.target.closest(".contribute-btn") && expenseId) {
      try {
        ...
        // If the current user already has a contribution, their old value will be
        // replaced (not added). So the effective max is balance + their existing amount.
        const myExistingContribution = contributions.find(c => c.groupMemberId === currentMemberId);
        const effectiveMax = balance + (myExistingContribution ? myExistingContribution.value : 0);
        ...
        ).textContent = `${effectiveMax.toFixed(2)} ${expense.currency}`;

        const contributionInput = document.getElementById("contribution-value");
        contributionInput.max = effectiveMax.toFixed(2);
  ...
  if (contributionForm) {
    contributionForm.addEventListener("submit", async (e) => {
      ...
      if (contributionValue > effectiveMax) {
        alert(
          `Your contribution cannot exceed ${effectiveMax.toFixed(2)}.`
        );
        return;
      }
