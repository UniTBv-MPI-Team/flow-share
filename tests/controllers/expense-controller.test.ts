...
const mockGroupMemberService =
  require("../../src/services/group-member-service") as jest.Mocked<
    typeof GroupMemberService
  >;
...
describe("Expense Controller", () => {
  ...
  beforeEach(() => {
    ...
    mockGroupMemberService.getMemberByUserAndGroup.mockResolvedValue(MOCK_ADMIN_MEMBER as any);
  });
  ...
  describe("handleUpdate", () => {
    ...
    it("should return 403 if user is not a group admin", async () => {
      mockGroupMemberService.getMemberByUserAndGroup.mockResolvedValue({ ...MOCK_ADMIN_MEMBER, isAdmin: false } as any);
      const req = createAuthenticatedRequest(routeParams, updateBody);
      const res = mockResponse();
      await ExpenseController.handleUpdate(req, res);
