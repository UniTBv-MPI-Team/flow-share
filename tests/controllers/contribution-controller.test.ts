...
const MOCK_CONTRIBUTION_WITH_MEMBER = {
  ...MOCK_CONTRIBUTION,
  member: { userId: AUTHENTICATED_USER_ID },
};
...
describe("Contribution Controller", () => {
  beforeEach(() => {
    ...
    mockContributionService.getContributionById.mockResolvedValue(MOCK_CONTRIBUTION_WITH_MEMBER);
    mockExpenseService.getExpenseById.mockResolvedValue(MOCK_EXPENSE as any);
  });
  ...
  describe("handleUpdate", () => {
    ...
    it("should return 404 if contribution is not found", async () => {
      mockContributionService.getContributionById.mockResolvedValue(null);
    ...
    it("should return 403 if user does not own the contribution", async () => {
      mockContributionService.getContributionById.mockResolvedValue({
        ...MOCK_CONTRIBUTION_WITH_MEMBER,
        member: { userId: 999 },
      });
      const req = createAuthenticatedRequest(routeParams, validBody);
      const res = mockResponse();
